...
void Statistics::Show()
{
    if (set.dbg.showStatistics)
    {
        ...
        char buffer[100] = {0};
        std::strcpy(buffer, SU::Int2String(static_cast<int>(CPU::GetFPS() + 0.5F), false, 3).c_str()); //-V2513
        std::strcat(buffer, "/"); //-V2513
        std::strcat(buffer, SU::Int2String(timeFrame, false, 3).c_str()); //-V2513
        std::strcat(buffer, "/"); //-V2513
        std::strcat(buffer, SU::Int2String(timeAllFrames, false, 3).c_str()); //-V2513
