...
void HAL_LTDC::Init()
{
    ...
    HAL_GPIO_Init(GPIOB, &initStr); //-V2571

    HAL_GPIO_WritePin(GPIOB, GPIO_PIN_6, GPIO_PIN_SET);         // Включение подсветки //-V2571
...
static void SetBuffers()
{
    ...
    pLayerCfg.FBStartAdress = reinterpret_cast<uint>(frontBuffer); //-V2571
...
void HAL_LTDC::ToggleBuffers()
{
    ...
    if (HAL_DMA2D_Init(&hDMA2D) == HAL_OK)
    {
        ...
        {
            if (HAL_DMA2D_Start(&hDMA2D, reinterpret_cast<uint>(backBuffer), reinterpret_cast<uint>(frontBuffer), SCREEN_WIDTH, SCREEN_HEIGHT) == HAL_OK) //-V2571
            {
...
void HAL_LTDC::FillRegion(int, int, int, int, Color color)
{
    ...
    handle.Instance = DMA2D; //-V2571

    if (HAL_DMA2D_Init(&handle) == HAL_OK)
    {
        if (HAL_DMA2D_ConfigLayer(&handle, 1) == HAL_OK)
        {
            if (HAL_DMA2D_Start(&handle, color.value, reinterpret_cast<uint>(backBuffer), SCREEN_WIDTH, SCREEN_HEIGHT) == HAL_OK) //-V2571
            {
