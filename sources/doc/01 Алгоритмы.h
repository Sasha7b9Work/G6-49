...
/*! \page page01 1. Алгоритмы
    ...
    * Число в квадратных скобках означает количество байт.
	...
	Обмен данными производится пакетами размером 64 байта.
	
	Структура пакета.
	
	\code
					+--------+--------------+-----------------+----------------------+---------+
					| CRC32  | Номер пакета | Номер субпакета | Кол-во байт данных   | Данные  |
	+---------------+--------+--------------+-----------------+----------------------+---------+
    | Номера байтов | 0...3  |    4...7     |     8...11      |       12...15        | 16...63 |
    +---------------+--------+--------------+-----------------+----------------------+---------+
	\endcode
    
    В поле <b>CRC32</b> находится контрольная сумма байтов, начиная с 4 по 63-й.
    
    <b>Номер пакета</b> - порядковый номер пересылаемого пакета. Пакеты имеют свою нумерацию для передатчика и приёмника.

    Процесс засылки информации в прибор происходит следующим образом.

    Панель ожидает прихода сигнала Ready() от процессора. Процесс передачи начинается после перехода сигнала Busy() в состояние Ready(). Панель передаёт 
    пакет. ПЛИС принимает его, вычисляет контрольную сумму и сравнивает с той, что в пакете. Затем формирует новый пакет, идентичный принятому (но с новой
    контрольной суммой) и возвращает его обратно. Затем снимает сигнал Ready() - транзакция закончена. Панель сравнивает переданный и принятый пакеты. В 
    случае их различия передача повторяется. В том случае, если пакет с таким номером уже принят (это возможно, если в процессе подтверждения произвошла 
	ошибка и панель шибочно считает, что передачу нужно повторить), то он просто отбрасывается прибором (отсылается панели в том виде, в котором принят).
