...
/*! \page page01 1. Алгоритмы
    ...
    * Число в квадратных скобках означает количество байт.
    ...
    \subsection subsec131 3.1 Передача данных от основного процессора.

    1. Послать количество передаваемых байт в ПЛИС.
    2. Передавать информацию порциями по 1024 байта (или меньше).

    \subsection subsec132 3.2 Приём данных от ПЛИС

    1. Для того, чтобы ПЛИС имел возможность передать данные основному процессору, основной процессор периодически передаёт ему команду RequestData.
    2. Далее принимается два байта, которые представляют собой шестнадцатиричное значение, означающее количество байт, готовых к передаче.
    3. Эти данные принимаются блоками по 1024 байта (или меньше).
